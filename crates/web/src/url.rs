...
#[derive(Debug)]
pub enum Error {
    DbConnectionFailed(deadpool_sqlite::rusqlite::Error),
...
pub async fn insert_url(
    ...
    tracing::trace!("Attempting to insert: {:#?}", data);

    let conn = db_handle.get_write_conn().await.unwrap();

    let entry = conn.interact(|conn| {
        let statement = conn.prepare(sql::INSERT_URL).unwrap();

        statement.query_row(params![data.identifier, data.url], |row| todo!());
    });
...
//    let client = db_handle.client().await?;
//    let data = client
...
//    let url = split_url.fold(scheme, |mut acc, chunk| {
//        acc.push_str("://");
//        acc.push_str(chunk);
//        acc
//    });
